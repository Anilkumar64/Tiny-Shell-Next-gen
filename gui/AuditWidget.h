...
// Audit log viewer — streams mock events; replace with ZkAuditTrail /
// StructuredAuditLogger API calls when backend exposes them.
...
public:
  explicit AuditWidget(tsh::ApiClient *api, QWidget *parent = nullptr)
...
private:
  void buildUi() {
    ...
    connect(pauseBtn, &QPushButton::toggled, this, [this, pauseBtn](bool p) {
      ...
      m_liveLbl->setStyleSheet(
          QString("font-size:12px; color:%1; font-weight:600;")
              .arg(p ? "#8891b0" : "#4ade80"));
    ...
    connect(m_searchEdit, &QLineEdit::textChanged, this,
            &AuditWidget::applyFilter);

    m_severityCb = new QComboBox(this);
    m_severityCb->addItems(
        {"All Severities", "INFO", "WARN", "ERROR", "CRITICAL"});
    ...
    m_table->setHorizontalHeaderLabels({"Timestamp", "Severity", "User",
  ...
  void appendEntry(const AuditEntry &e) {
    ...
    QFont f;
    f.setBold(e.severity == "CRITICAL" || e.severity == "ERROR");
    sevItem->setFont(f);
  ...
  void applyFilter() {
    ...
    const QString sevFilter =
        m_severityCb->currentIndex() == 0 ? "" : m_severityCb->currentText();
    const QString outFilter =
        m_outcomeCb->currentIndex() == 0 ? "" : m_outcomeCb->currentText();
  ...
  void seedEvents() {
    ...
    const QList<Seed> seeds = {
        {"INFO", "alice", "EXECUTE_COMMAND", "grep_pipeline", "SUCCESS",
         "platform-eng"},
        {"INFO", "bob", "MANAGE_JOBS", "job-j-0003", "SUCCESS", "data-science"},
        {"WARN", "eve", "READ_AUDIT_LOG", "/audit/stream", "DENIED",
         "platform-eng"},
        {"ERROR", "frank", "EXECUTE_PIPELINE", "sort_pipeline", "FAILED",
         "data-science"},
        {"INFO", "carol", "READ_CLUSTER_STATUS", "/cluster/health", "SUCCESS",
         "security"},
        {"CRITICAL", "unknown", "EXECUTE_COMMAND", "rm_pipeline", "DENIED",
         "qa-automation"},
        {"INFO", "dave", "MANAGE_TENANTS", "qa-automation", "SUCCESS",
         "qa-automation"},
        {"WARN", "alice", "MANAGE_RESOURCES", "worker-02", "SUCCESS",
         "platform-eng"},
    };
    for (int i = seeds.size() - 1; i >= 0; --i) {
      const auto &s = seeds[i];
      appendEntry({QDateTime::currentDateTime()
                       .addSecs(-(seeds.size() - i) * 45)
  ...
  void appendRandomEvent() {
    static const QStringList users{"alice", "bob", "carol",
                                   "dave",  "eve", "unknown"};
    static const QStringList actions{"EXECUTE_COMMAND", "READ_SYSTEM_METRICS",
                                     "MANAGE_JOBS", "READ_AUDIT_LOG",
                                     "EXECUTE_PIPELINE"};
    static const QStringList sevs{"INFO", "INFO",  "INFO",
                                  "WARN", "ERROR", "CRITICAL"};
    static const QStringList outs{"SUCCESS", "SUCCESS", "SUCCESS", "DENIED",
                                  "FAILED"};
    static const QStringList tenants{"platform-eng", "data-science", "security",
                                     "qa-automation"};
    static const QStringList res{"grep_pipeline", "job-queue", "worker-01",
                                 "/metrics", "/exec"};

    auto pick = [](const QStringList &l) {
      return l[QRandomGenerator::global()->bounded(l.size())];
    };
    appendEntry({QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss"),
                 pick(sevs), pick(users), pick(actions), pick(res),
                 pick(tenants), pick(outs)});
    applyFilter();
  }

...
// Data is kept in-process; a real backend endpoint would replace the in-memory map.
...
public:
    explicit ClusterWidget(QWidget* parent = nullptr) : QWidget(parent) {
        buildUi();
...
private:
    void buildUi() {
        ...
        connect(refreshBtn, &QPushButton::clicked, this, &ClusterWidget::refreshStats);
        ...
        m_table->setHorizontalHeaderLabels({
            "Host", "Port", "Status", "Health Score", "OK Jobs", "Failed Jobs", "Latency ms"
        });
        ...
        connect(m_removeBtn, &QPushButton::clicked, this, &ClusterWidget::removeSelected);

        connect(m_table, &QTableWidget::itemSelectionChanged, this, [this]{
            m_removeBtn->setEnabled(m_table->currentRow() >= 0);
            m_selInfoLbl->setText(m_table->currentRow() >= 0 ?
    ...
    static QLabel* makeStatCard(const QString& label, const QString& val,
                                QHBoxLayout* row, const QString& color = "#e8eaf0")
    {
        ...
        valLbl->setStyleSheet(QString("font-size:22px; font-weight:700; color:%1;").arg(color));
        auto* lblLbl = new QLabel(label, frame);
        lblLbl->setStyleSheet("font-size:10px; color:#8891b0; font-weight:600; letter-spacing:0.5px;");
        vl->addWidget(valLbl);
        vl->addWidget(lblLbl);
    ...
    void addNodeRow(const NodeRow& n) {
        ...
        auto setItem = [&](int col, const QString& text, const QString& color = "#e8eaf0") {
        ...
        // Status badge
        auto* statusItem = new QTableWidgetItem(n.healthy ? "● Healthy" : "● Unreachable");
        statusItem->setForeground(QColor(n.healthy ? "#4ade80" : "#f87171"));
        statusItem->setFont([]{QFont f; f.setBold(true); return f;}());
    ...
    void refreshStats() {
        ...
        const int healthy = std::count_if(m_nodes.begin(), m_nodes.end(),
                                          [](const NodeRow& n){ return n.healthy; });
        const int dead    = total - healthy;
        double avgScore   = 0;
        for (const auto& n : m_nodes) avgScore += n.healthScore;
        if (total) avgScore /= total;
    ...
    void showAddDialog() {
        ...
        portSpin->setRange(1, 65535); portSpin->setValue(4445);
        ...
        if (dlg->exec() == QDialog::Accepted) {
            NodeRow nr{hostEdit->text().trimmed(), portSpin->value(),
                       50.0, true, 0, 0, 0.0};
            addNodeRow(nr);
            refreshStats();
        }

...
public:
    explicit RbacWidget(QWidget* parent = nullptr) : QWidget(parent) {
        buildUi();
        seedDefaultRoles();
    }
...
private:
    void buildUi() {
        ...
        m_roleList->setStyleSheet(
            ...
            "QListWidget::item { padding:10px 12px; border-radius:6px; color:#e8eaf0; }"
        ...
        connect(m_roleList, &QListWidget::currentRowChanged,
                this, &RbacWidget::onRoleSelected);
        ...
        connect(addRoleBtn, &QPushButton::clicked, this, &RbacWidget::showAddRoleDialog);
        ...
        connect(delRoleBtn, &QPushButton::clicked, this, &RbacWidget::deleteCurrentRole);
        ...
        permHdr->setStyleSheet(
            "font-size:10px; font-weight:600; letter-spacing:1px; color:#4a5068; margin-top:8px;");
        ...
        for (const QString& perm : allPermissions()) {
            ...
            cb->setStyleSheet(
                QString("QCheckBox { color:#e8eaf0; font-family:'JetBrains Mono','Consolas',mono;"
                        " font-size:12px; }"
    ...
    void seedDefaultRoles() {
        ...
        addRole("admin",    "Full system administrator",
            {"ADMIN"});
        ...
        addRole("viewer",   "Read-only system access",
    ...
    void onRoleSelected(int row) {
        if (row < 0) { setPermEditorEnabled(false); return; }
        const QString name = m_roleList->item(row)->text();
        if (!m_roles.contains(name)) return;
        m_currentRole = name;
    ...
    void onPermissionToggled() {
        // If ADMIN checked, check all; if ADMIN unchecked, uncheck all if applicable
        if (m_currentRole.isEmpty()) return;
        ...
        if (adminCb && adminCb->isChecked()) {
            ...
            for (auto* cb : m_permChecks) cb->setEnabled(true);
        }
    }
        ...
        if (dlg->exec() == QDialog::Accepted) {
            ...
            if (n.isEmpty() || m_roles.contains(n)) {
                QMessageBox::warning(this, "Error",
                    n.isEmpty() ? "Name cannot be empty." : "Role already exists.");
                dlg->deleteLater(); return;
            }
    ...
    void setPermEditorEnabled(bool on) {
        ...
        for (auto* cb : m_permChecks) cb->setEnabled(on);
    }

...
public:
    explicit JobWidget(QWidget* parent = nullptr) : QWidget(parent) {
        buildUi();
        seedJobs();
...
private:
    void buildUi() {
        ...
        submitPanel->setStyleSheet(
            "QFrame { background:#1c1f2b; border:1px solid #2a2d3e; border-radius:10px; }");
        auto* sv = new QVBoxLayout(submitPanel);
        sv->setContentsMargins(16,16,16,16); sv->setSpacing(12);

        auto* fmHdr = new QLabel("SUBMIT JOB", submitPanel);
        fmHdr->setStyleSheet("font-size:10px; font-weight:600; letter-spacing:1px; color:#4a5068;");
        sv->addWidget(fmHdr);
        ...
        m_sfRadio  = new QRadioButton("Speculative Fan-Out  (needs ≥2 nodes)", submitPanel);
        m_bftRadio = new QRadioButton("Byzantine Fault Tolerant  (needs ≥3 nodes)", submitPanel);
        ...
        for (auto* rb : {m_rrRadio, m_sfRadio, m_bftRadio}) {
            rb->setStyleSheet(
                "QRadioButton { color:#e8eaf0; font-size:12px; }"
                "QRadioButton::indicator:checked { background:#00b4d8; border-color:#00b4d8; }");
            sv->addWidget(rb);
        }

        // Description boxes
        m_stratDescLbl = new QLabel(routingDesc(RoutingMode::RoundRobin), submitPanel);
        ...
        auto updateDesc = [this]{
            RoutingMode m = m_rrRadio->isChecked() ? RoutingMode::RoundRobin :
                            m_sfRadio->isChecked() ? RoutingMode::Speculative : RoutingMode::BFT;
        ...
        rv->setContentsMargins(16,16,16,16); rv->setSpacing(10);

        auto* queueHdrRow = new QHBoxLayout;
        auto* queueHdr = new QLabel("JOB QUEUE", rightPanel);
        queueHdr->setStyleSheet("font-size:10px; font-weight:600; letter-spacing:1px; color:#4a5068;");
        ...
        connect(clearDoneBtn, &QPushButton::clicked, this, &JobWidget::clearDoneJobs);
    ...
    void seedJobs() {
        addJob({"j-0001", "grep_pipeline",    "Done",    "worker-01", jobTime(-120), RoutingMode::RoundRobin});
        addJob({"j-0002", "top_ten_procs",    "Done",    "worker-02", jobTime(-90),  RoutingMode::Speculative});
        addJob({"j-0003", "audit_query",      "Running", "worker-01", jobTime(-30),  RoutingMode::RoundRobin});
        addJob({"j-0004", "critical_cleanup", "Running", "worker-03", jobTime(-15),  RoutingMode::BFT});
        addJob({"j-0005", "mem_snapshot",     "Pending", "",          jobTime(0),    RoutingMode::RoundRobin});
    }
    ...
    void refreshRow(int row) {
        ...
        if (j.status=="Running") statusColor="#fbbf24";
        else if (j.status=="Done") statusColor="#4ade80";
        else if (j.status=="Failed") statusColor="#f87171";
        else if (j.status=="Pending") statusColor="#00b4d8";
        if (!m_table->item(row,4)) m_table->setItem(row,4,new QTableWidgetItem);
    ...
    static QString routingLabel(RoutingMode m) {
        switch (m) {
        case RoutingMode::Speculative: return "Speculative";
        case RoutingMode::BFT:         return "BFT";
    ...
    static QString routingDesc(RoutingMode m) {
        ...
        case RoutingMode::Speculative:
            return "Sends the AST to 2 nodes simultaneously. Returns whichever result arrives first. "
                   "Requires ≥2 registered nodes.";
        case RoutingMode::BFT:
            return "Byzantine Fault Tolerant execution on 3 nodes. Majority result wins. "
                   "Requires ≥3 registered nodes. Higher latency, maximum integrity.";
        default:
            return "Distributes jobs across nodes in round-robin order using the sharded scheduler. "
    ...
    void scheduleJob() {
        ...
        if (name.isEmpty()) { m_astEdit->setFocus(); return; }
    ...
    void tickJobs() {
        static const QStringList nodes{"worker-01","worker-02","worker-03","localhost"};
        bool changed = false;
        for (auto& j : m_jobs) {
            if (j.status=="Pending") {
                j.status="Running";
                ...
                if (QRandomGenerator::global()->bounded(100) < 40) {
                    j.status = QRandomGenerator::global()->bounded(10)<1 ? "Failed" : "Done";
                    changed=true;
                }
            }
        }
        if (changed) {
            for (int r=0; r<m_jobs.size(); ++r) refreshRow(r);
            updateDepthLabel();
        }
    }
    ...
    void clearDoneJobs() {
        for (int r=m_table->rowCount()-1; r>=0; --r) {
            if (r < m_jobs.size() && (m_jobs[r].status=="Done"||m_jobs[r].status=="Failed")) {
                m_table->removeRow(r);
                m_jobs.removeAt(r);
            }
        }
    ...
    void updateDepthLabel() {
        const int pending = std::count_if(m_jobs.begin(),m_jobs.end(),
            [](const Job& j){ return j.status=="Pending"||j.status=="Running"; });

  ...
  std::thread receiver([&]() {
    while (active.load()) {
      ...
      if (resp_type == SecureChannel::MsgType::PTY_OUTPUT ||
        ...
        std::cout.write(reinterpret_cast<const char *>(resp.data()),
                        static_cast<std::streamsize>(resp.size()));
        std::cout.flush();
      }
...
void Client::run() {
  ...
  if (sc.handshake(SecureChannel::Mode::CLIENT)) {
    ...
    if (!sc.send_message(std::vector<uint8_t>(hello.begin(), hello.end()),
    ...
    if (!sc.receive_message(hello_resp, hello_type)) {
      std::cerr
          << "Connection closed before controller identity was acknowledged.\n";
      return;
    }
    ...
    while (true) {
      ...
      // Removed: steganographic command channels have no place in a legitimate
      // production client.
      ...
      // Removed: arbitrary remote binary execution is disabled outside
      // controlled research builds.
